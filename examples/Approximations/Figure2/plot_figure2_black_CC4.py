"""Figure 2 — Monophasic stimulation pulse overview (CC4 model).

Four-panel figure (2x2):
  A: FFT coefficient amplitudes (stem plot, log x-axis)
  B: CC4 dielectric properties with frequency-skipping check
     (white=needed, magenta=skipped)
  C: CC4 ratio between real and imaginary conductivity with frequency-skipping check
...
for material, parameters in default_dielectric_parameters["ColeCole4"].items():
    if material in ["Blood", "Unknown"]:
        continue
    material_modelCC4[material] = modelCC4(parameters)
...
# ============================================================
# Panel B — CC4 dielectric properties
# ============================================================
plot_dielectric_panel(ax_b, material_modelCC4, frequencies, threshold, "B")

# ============================================================
# Panel C — Ratio between real and imaginary part (CC4)
# ============================================================
full_cc4, needed_cc4, not_needed_cc4 = compute_dielectric_classification(
    material_modelCC4, frequencies, threshold
)
ax_c.set_xscale("log")
ax_c.set_yscale("log")
for mat in material_modelCC4:
    if mat == "CSF":
        continue
    ratio = full_cc4[mat].real / np.abs(full_cc4[mat].imag)
    ratio_needed = needed_cc4[mat].real / np.abs(needed_cc4[mat].imag)
    ratio_not_needed = not_needed_cc4[mat].real / np.abs(not_needed_cc4[mat].imag)
...
# ---------- Save ----------
fig.tight_layout()
fig.savefig("figure2_black_CC4.pdf", bbox_inches="tight")
fig.savefig("figure2_black_CC4.svg", bbox_inches="tight")
plt.close()
print("Saved figure2_black_CC4.pdf / .svg")
