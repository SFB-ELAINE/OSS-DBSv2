...
for idx, frequency in enumerate(frequencies):
    ...
    permittivity[idx] = material_model["Gray matter"].complex_permittivity(omega)

for idx, frequency in enumerate(frequencies):
    omega = 2 * np.pi * frequency
    conductivity[idx] = material_model["Gray matter"].complex_conductivity(omega)

permittivity_bench, conductivity_bench = get_benchmark("greymatter", frequencies)
plt.xscale("log")
