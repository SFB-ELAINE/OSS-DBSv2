...
def apply_dark_style(fig, axes):
    """Make background dark and axes and other lines white."""
...
fig.tight_layout()
fig.savefig("impedance_spectrum_bode_black.pdf", facecolor="black")
fig.savefig("impedance_spectrum_bode_black.svg", facecolor="black")
plt.show()
...
fig2.tight_layout()
fig2.savefig("impedance_spectrum_decomposition_black.pdf", facecolor="black")
fig2.savefig("impedance_spectrum_decomposition_black.svg", facecolor="black")
plt.show()
...
fig3.tight_layout()
fig3.savefig("impedance_spectrum_nyquist_black.pdf", facecolor="black")
fig3.savefig("impedance_spectrum_nyquist_black.svg", facecolor="black")
plt.show()

# --- Decomposition plot for the matching parameter set (Day 1) paper style ---
fig2, ax2_mag = plt.subplots(figsize=(7, 6))

ref_name = "Day 1"
markevery = max(1, len(sim_frequencies) // 20)
ax2_mag.plot(sim_frequencies, np.abs(sim_no_interface_Z), label="VCM only", ls="dashed")
ax2_mag.plot(
    sim_frequencies,
    np.abs(sim_Z),
...
ax2_mag.legend()
ax2_mag.grid(True, which="both", ls=":", alpha=0.5, color="0.4")
ax2_mag.set_yticks([5e3, 1e4, 2e4, 5e4, 10e4])

apply_dark_style(fig2, [ax2_mag])
fig2.tight_layout()
fig2.savefig("impedance_spectrum_decomposition_paper_black.pdf", facecolor="black")
fig2.savefig("impedance_spectrum_decomposition_paper_black.svg", facecolor="black")
plt.show()
