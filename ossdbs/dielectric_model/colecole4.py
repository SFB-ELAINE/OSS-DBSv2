...
class ColeColeParameters:
    ...
    def assert_order(self, order: int) -> bool:
        return (len(self.alpha) == order) and\
               (len(self.tau) == order) and\
               (len(self.eps_delta) == order)


WhiteMatterColeCole4Default =\
    ColeColeParameters(alpha=np.array([0.1, 0.1, 0.3, 0.02]),
...
CSFColeCole4Default =\
    ColeColeParameters(alpha=np.array([0.1, 0.0, 0.0, 0.0]),
...
BloodColeCole4Default =\
    ColeColeParameters(alpha=np.array([0.1, 0.1, 0.0, 0.0]),
...
class ColeCole4Model(DielectricModel):
    ...
    def complex_permittivity(self, omega: float) -> complex:
        ...
        """
        divisor = 1 + (1j * omega * self._parameters.tau) ** (1 - self._parameters.alpha)
        eps_dispersion = self._parameters.eps_inf + np.sum(self._parameters.eps_delta / divisor)
