...
class VolumeConductorFloating(VolumeConductor):
    ...
    def __create_space(self):
        ...
        boundaries_nonfloating = [contact.name for contact in self.model_geometry.contacts.unused]
        boundaries_nonfloating.extend(boundaries)
        spaces_field = [self.h1_space(boundaries),
                        self.surfacel2_space(boundaries_nonfloating)]
        spaces_floating = [self.number_space()
