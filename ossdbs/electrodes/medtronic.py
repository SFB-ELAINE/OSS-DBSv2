...
class MedtronicModel(ElectrodeModel):
    ...
    def _construct_encapsulation_geometry(
        ...
        # Unlike Cylinder, Sphere has no direction parameter, so its BREP
        # seam always sits at a fixed orientation regardless of
        # self._direction. Pre-rotate it around its own center (a no-op on
        # its shape, since a sphere is symmetric about its center) so the
        # seam doesn't happen to land somewhere that breaks Netgen's
        # surface mesher for a given electrode direction. Same technique
        # as BrainGeometry._create_sphere/_create_ellipsoid.
    ...
    def __body(self) -> netgen.libngpy._NgOCC.TopoDS_Shape:
        ...
        tip = occ.Sphere(c=center, r=radius)
        # See matching comment in _construct_encapsulation_geometry.
        tip = tip.Rotate(occ.Axis(p=center, d=(0, 1, 0)), 90)
        height = self._parameters.total_length - self._parameters.tip_length
        lead = occ.Cylinder(p=center, d=self._direction, r=radius, h=height)
        body = tip + lead
