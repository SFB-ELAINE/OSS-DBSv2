...
from .neuron_model import MRG2002, McNeal1976
...
class NeuronStimulation:
    ...
    def __init__(
        ...
        self.N_neurons = pathway_dict[
            "N_seeded_neurons"
        ]  # int, number of neurons in the pathway
        self.orig_N_neurouns = pathway_dict[
            "N_orig_neurons"
        ]  # int, number of neurons per pathway as defined in the connectome (before Kuncel pre-filtering)
        ...
        # we always simulate only one pulse from DBS.
        # If more needed, we should just copy the array and make sure the time vectors are in order
        self.n_pulse = 1
        self.scaling_vector = scaling_vector  # superposition-based solution scaling (contact-wise vector!)
        self.scaling = signal_dict[
            "scaling"
        ]  # float, simple scaling of the whole solution (not contact-wise!)
        ...
        self.axon_morphology = get_axon_morphology(
            axonNEURONModel, self.axonDiam, None, self.nRanvier
        )
    ...
    def run_NEURON(self, v_ext):
        ...
        ----------
        v_ext: numpy.ndarray,
            NxM extracellular potential distribution (mV) in space (on the neuron) and time (DBS signal)

        Returns
        -------
        spike: bool
            flag for induced action potential
        """
        # neuron simulation
        if self.axonModel == "McNeal1976":
            v_init = -70.0
            n.h('{load_file("init_B5_extracellular.hoc")}')
        elif self.axonModel in ["MRG2002", "MRG2002_DS"]:
            v_init = -80.0
            ...
            _logger.critical("The NEURON model is not supported")
            raise SystemExit
    ...
    def get_v_ext(self, v_time_sol):
        """Convert potential to extracellular potential.

        Parameters
        ----------
        v_time_sol: numpy.ndarray
            NxM (abstract) potential distribution (in V) in space (on the neuron) and time (DBS signal)
        ...
        scale_factor = 1000.0 * self.scaling

        # just convert to mV
        if self.scaling_vector is None:
            v_ext = v_time_sol * scale_factor  # convert to mV
            return v_ext

        for contact_i in range(len(self.scaling_vector)):
            # TODO Why "v_ext +"? v_ext is zero?!
            v_ext = v_ext + v_time_sol * scale_factor * self.scaling_vector[contact_i]
        return v_ext
    ...
    def modify_hoc_file(self):
        ...
        """
        if self.axonModel in ["MRG2002", "MRG2002_DS"]:
            neuron_model = MRG2002()
            ...
            neuron_model.paste_to_hoc(
                self.nRanvier,
                self.axon_morphology["n_para1"],
                self.axon_morphology["n_para2"],
                axoninter,
                self.axon_morphology["n_segments"],
                v_init,
                self.axonDiam,
                self.axon_morphology["para1_length"] * 1e3,
                self.axon_morphology["para2_length"] * 1e3,
                self.axon_morphology["ranvier_length"] * 1e3,
                self.axon_morphology["node_d"],
                self.axon_morphology["axon_d"],
                self.axon_morphology["para1_d"],
                self.axon_morphology["para2_d"],
                self.axon_morphology["node_step"] * 1e3,
                self.axon_morphology["lamellas"],
                self.stepsPerMs,
            )

        elif self.axonModel == "McNeal1976":
            neuron_model = McNeal1976()
    ...
    def create_leaddbs_outputs(self, Axon_Lead_DBS):
        ...
        Axon_Lead_DBS: numpy.ndarray
            NxM, geometry, index and activation status of neurons (equivalent of connectome.fibers format in Lead-DBS)
        ...
        if self.scaling_index is None:
            if self.pathway_name is None:
                ...
                savemat(
                    self.folder_to_save + "/Axon_state_" + self.pathway_name + ".mat",
                    mdic,
                )
        else:
            if self.pathway_name is None:
    ...
    def create_paraview_outputs(self, Axon_Lead_DBS):
        ...
        if self.scaling_index is None:
            if self.pathway_name is None:
                np.savetxt(
                    self.folder_to_save + "/Axon_state.csv",
                    Axon_Lead_DBS,
                    delimiter=",",
                ...
                np.savetxt(
                    self.folder_to_save + "/Axon_state_" + self.pathway_name + ".csv",
                    Axon_Lead_DBS,
                    delimiter=",",
                    header="x-pt,y-pt,z-pt,idx,status",
                )
    ...
    def store_axon_statuses(self, percent_activated, percent_damaged, percent_csf):
        ...
        if self.scaling_index is None:
            if self.pathway_name is None:
                with open(
                    self.folder_to_save + "/Pathway_status.json", "w"
                ) as save_as_dict:
    ...
    def check_pathway_activation(self, pathway_dataset):
        ...
        percent_activated = np.round(
            Activated_models / float(self.orig_N_neurouns) * 100, 2
        )
        percent_damaged = np.round(
            np.sum(np.isclose(pre_status, -1.0)) / float(self.orig_N_neurouns) * 100, 2
        )
        percent_csf = np.round(
            np.sum(np.isclose(pre_status, -2.0)) / float(self.orig_N_neurouns) * 100, 2
        )
