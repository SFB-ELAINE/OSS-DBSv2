...
class AxonModels:
    ...
    def _import_leaddbs_neurons(self, hemis_idx):
        ...
        if "neuronModel" in file_inp["settings"]:
            ...
            if self.axon_model not in ["MRG2002", "MRG2002_DS", "McNeal1976"]:
                raise ValueError(
                    "The selected NEURON models is not recognized,"
                    "check oss-dbs_parameters.mat"
                )
        else:
        ...
        if stimSets:
            stim_protocols = np.genfromtxt(
                os.environ["STIMDIR"] + "/Current_protocols_" + str(hemis_idx) + ".csv",
                dtype=float,
                delimiter=",",
                names=True,
            )
            ...
            for j in range(total_protocols):
                ...
                for i in range(total_contacts):
                    if not math.isnan(protocols_array[j, i]):
                        ampl_vector[
                            i
                        ] = 1.0  # you do not need a value, just substitute NaN
        else:
        ...
        _logger.info("Output_dictionary: ", self.output_directory)
    ...
    def convert_fibers_to_axons(self):
        ...
        for i in range(len(self.axon_diams_all)):
            ...
            axon_morphology = get_axon_morphology(
                self.axon_model, self.axon_diams_all[i], self.axon_lengths_all[i]
            )

            # multiple .mat files (manual input)
            if len(self.pathway_mat_file) > 1:
                (
                    n_Ranviers_per_projection_all[i],
                    n_Neurons_all[i],
        ...
        self._save_axon_parameters_in_json(
            n_Ranviers_per_projection, n_Neurons, orig_n_Neurons
        )

    def _save_axon_parameters_in_json(
        self, n_Ranviers_per_projection, n_Neurons, orig_n_Neurons
    ):
    ...
    def deploy_axons_fibers(
        ...
        if fiber_array.ndim == 1:
            print(
                projection_name,
                "projection is empty, check settings for fib. diameter and axon length",
            )
        ...
        np.savetxt(
            self.output_directory + "/" + "axon_array_2D_" + projection_name + ".csv",
            axon_array_2D,
            delimiter=" ",
        )
...
def get_axon_morphology(axon_model, axon_diam, axon_length=None, n_Ranviers=None):
    ...
    if "MRG2002" in axon_model:
        ...
        (
            ...
            nr["ranvier_length"] * 1e-3,
            nr["para1_length"] * 1e-3,
            nr["para2_length"] * 1e-3,
            nr["deltax"] * 1e-3,
        )
        ...
        if axon_length is not None:
            ...
            axon_morphology["n_Ranviers"] = int(
        ...
        if axon_morphology["n_Ranviers"] % 2 == 0:
            ...
            axon_morphology["axon_length"] = (
                axon_morphology["n_Ranviers"] * axon_morphology["node_step"]
            )

        axon_morphology["n_para1"] = (
            nr["para1_nodes"] * (axon_morphology["n_Ranviers"] - 1) / (21 - 1)
        )
        axon_morphology["n_para2"] = (
            nr["para2_nodes"] * (axon_morphology["n_Ranviers"] - 1) / (21 - 1)
        )

        axon_morphology["n_segments"] = int(
            (axon_morphology["n_Ranviers"] - 1) * axon_morphology["n_comp"] + 1
...
def place_axons_on_streamlines(
    ...
    for inx_axn in range(len(streamlines_resampled)):
        single_streamline_ROI_centered = np.zeros(
            (axon_morphology["n_Ranviers"], 3), float
        )
...
def get_local_compartment_coords(axon_morphology):
    ...
    if (
        axon_morphology["axon_model"] == "MRG2002"
        and axon_morphology["axon_diam"] >= 5.7
    ):
        ...
        axon_morphology["axon_model"] == "MRG2002_DS"
        and axon_morphology["axon_diam"] >= 5.7
    ):
        # node -- -- internodal -- -- -- -- internodal -- -- node
