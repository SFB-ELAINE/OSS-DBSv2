...
- ``RelativeTolerance`` (default: 1e-8) — relative residual tolerance. For most
...
- ``AbsoluteTolerance`` (default: 1e-8; accepts ``null`` to disable) — optional
  positive absolute residual tolerance. When both tolerances are configured,
  NGSolve stops when the residual reaches the larger of
  ``initial residual * RelativeTolerance`` and ``AbsoluteTolerance``.
