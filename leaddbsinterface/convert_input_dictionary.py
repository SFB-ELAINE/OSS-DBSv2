...
def main():
    ...
    """
    parser = argparse.ArgumentParser(prog="Lead-DBS to OSS-DBS converter",
                                     description="""Converts mat-files written by Lead-DBS
                                                    to OSS-DBS v2 input dictionaries.""")
    parser.add_argument('leaddbs_dictionary', type=str,
                        help="input dictionary in mat format provided by Lead-DBS"
                        )
    parser.add_argument('--hemi_side', type=int, choices=[0, 1], required=True,
    ...
    settings = ls.make_oss_settings(hemis_idx=args.hemi_side, output_path=args.output_path)
