...
class TestComparePathways:
    """Tests for compare_pathways function (requires sklearn)."""
    ...
    def mock_dataframes(self):
        ...
        df1 = pd.DataFrame({"status": [1, 1, 0, -1, 0]})
        df2 = pd.DataFrame({"status": [1, 0, 0, -1, 1]})
        return df1, df2

    def test_compare_pathways_returns_metrics(self, mock_dataframes):
        """Test that compare_pathways returns expected metrics."""
        pytest.importorskip("sklearn")
        from ossdbs.axon_processing.utilities import compare_pathways

        df1, df2 = mock_dataframes
        result = compare_pathways(df1, df2)
    ...
    def test_compare_pathways_mismatched_shapes(self, mock_dataframes):
        ...
        df1 = pd.DataFrame({"status": [1, 1, 0]})
        df2 = pd.DataFrame({"status": [1, 0, 0, -1, 1]})

        with pytest.raises(ValueError, match="same number of axons"):
            compare_pathways(df1, df2)

    def test_compare_pathways_all_zeros(self):
        """Test that all-zero activation returns None."""
        pytest.importorskip("sklearn")
        import pandas as pd

        from ossdbs.axon_processing.utilities import compare_pathways

        df1 = pd.DataFrame({"status": [0, 0, 0]})
        df2 = pd.DataFrame({"status": [0, 0, 0]})
